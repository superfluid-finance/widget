...
* [Building your Widget using JSON](#using-json)
* [Building your Widget using the UI](#using-ui)
...
- **paymentOptions**: An array of available payment options.
  ...
  - **flowRate**: Specifies the rate at which payments (streams) flow.
...
For those who prefer a more visual approach or might not be as comfortable with JSON configurations, the Superfluid Checkout Widget also offers a UI-based setup.
...
2. **Define Product Details:** Use the form fields provided to enter the name, description, image URI, success text, and success URL for your product.
    
3. **Specify Payment Options:** Add one or more payment options by providing the receiver's Ethereum address, chain ID, super token details, and the desired flow rate. If you have predefined super tokens, you can select them from a dropdown list.
...
6. **Preview & Generate Code:** Once you've configured all the settings, you can preview the widget in real-time. If satisfied, click on "Generate Code" to receive the necessary code snippets to embed the widget into your app.
