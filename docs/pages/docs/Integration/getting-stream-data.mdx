...
Depending on your use case, there are two main ways to get stream data from Superfluid: via our SDK and our Subgraph.

## Fetching Stream Data Using SDK
...
let flowInfo = await daix.getFlow({ sender: string, receiver: string, providerOrSigner: ethers.providers.Provider | ethers.Signer });
```
...
<Callout> **Note**: Tokens can be loaded by symbol or address. While using the SDK is ideal for real-time interactions and single address checks, the Subgraph method is superior for querying data on multiple addresses based on specific criteria. </Callout>