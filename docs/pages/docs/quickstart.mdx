...
async function getFlowRate(userAddress) {
  ...
  const sf = await Framework.create({
    chainId: INSERT_CHAIN_ID_HERE, // Replace with the chain ID of your Ethereum network
    provider: myProvider,
  });
...
```
<Callout type="info">
Use EVM libraries (eg. [wagmi](https://wagmi.sh/) or [ethers](https://docs.ethers.org/)) to create the provider and retrieve the user's address. You can also use the [Superfluid console](https://console.superfluid.finance/ethereum/supertokens) to retrieve the Super Token address.
</Callout>
